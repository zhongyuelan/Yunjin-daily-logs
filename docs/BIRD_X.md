...
## 1. 什么是 Bird-X？
**Bird-X** 是 ClawX 项目中用于与 Twitter/X 交互的核心引擎。它是一个高性能的命令行工具（由 `bird` 封装），通过预配置的会话凭证（Cookie）直接与 Twitter/X 的 GraphQL API 通信。
...
## 3. 主要自动化任务
ClawX 的多个智能代理依赖 Bird-X 在后台静默运行：
...
## 5. 安全红线
**绝对禁止**：严禁 ClawX 在未经主人逐条授权的情况下，代表主账号通过 Bird-X 发布任何推文（`tweet`）或回复（`reply`）。所有自动化生成的内容仅允许发布在本地 Mini Twitter 系统中。
